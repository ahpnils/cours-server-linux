...
```
sudo apt -y install qemu-kvm libvirt-daemon-system libvirt-daemon virtinst bridge-utils libosinfo-bin virt-manager qemu-system virtiofsd
```

Libvirt crée son propre groupe, nommé "libvirt", mais sous Ubuntu libvirt est
configuré pour utiliser le groupe "kvm". Ajoutons donc l'utilisateur courant au
group "kvm" : 
```
